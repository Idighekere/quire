...
import { ErrorResponse, SuccessResponse, normalizeCourseCode, splitCourseCodePrefix } from "@/common/utils";
import { catchAsync } from "@/middlewares";
import { Book, Course } from "@/models";
import { driveServiceForAdmin } from "@/services/drive.service";
...
const syncDrive = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      return next(
        new ErrorResponse(
          "Failed to read the Drive folder. Check that an admin has connected Google.",
          500,
        ),
      );
    }
