...
import { ErrorResponse, SuccessResponse, isDriveFilePublic, normalizeCourseCode, splitCourseCodePrefix } from "@/common/utils";
import { catchAsync } from "@/middlewares";
import { Book, Course } from "@/models";
import { DriveService, driveServiceForAdmin, driveServiceForServiceAccount } from "@/services/drive.service";
 ...
 */
const deriveBookTitleFromFileName = (
  rawName: string,
  folderTitleFallback: string,
): string => {
...
interface ImportFileArgs {
  file: { id?: string | null; name?: string | null; size?: string | null };
  /** Folder-name segments from the root down to the file's parent. */
  segments: string[];
  category: BookCategory;
  addedBy: mongoose.Types.ObjectId | string;
  adminDriveService: DriveService;
  added: AddedEntry[];
  skipped: SkippedEntry[];
  coursesCreated: CreatedCourseEntry[];
  /** Picker imports verify anonymous readability; sync skips this (admin's own tree). */
  checkPublicity: boolean;
  /** When set and the file is private, try sharePublic (works on admin-owned files). */
  makePublic: boolean;
  /** Immediate parent folder ID — stored as course.driveFolderId when it is the course folder. */
 ...
 */
const importDriveFile = async (args: ImportFileArgs): Promise<void> => {
  const {
    file,
    segments,
    category,
  ...
  const { prefix } = splitCourseCodePrefix(rawCodeSegment);
  ...
  if (!course) {
    ...
    try {
      ...
      if (err && typeof err === "object" && "code" in err && err.code === 11000) {
        const winner = await Course.findOne({ courseCode });
        if (winner) {
          course = winner;
  ...
  const known = await Book.findOne({ driveFileId: file.id });
  ...
  if (checkPublicity) {
    let isPublic = false;
    try {
      isPublic = await isDriveFilePublic(file.id);
    } catch {
  ...
  const bookTitle = deriveBookTitleFromFileName(
    fileName,
    courseMatch[2].trim(),
  );
 ...
 */
const syncDrive = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    const rootFolderId = ENVIRONMENT.DRIVE.ROOT_FOLDER_ID;
    if (!rootFolderId) {
      return next(
        new ErrorResponse(
    ...
    const category =
      requestedCategory && VALID_CATEGORIES.includes(requestedCategory as BookCategory)
        ? (requestedCategory as BookCategory)
    ...
    const syncDriveService = driveServiceForServiceAccount();
    ...
    try {
      ...
      return next(
        new ErrorResponse(
          "Failed to read the Drive folder. Check that the library master folder is shared with the service account.",
          500,
        ),
      );
    }
 ...
 */
const getPickerToken = catchAsync(
  async (_req: Request, res: Response) => {
 ...
 */
const importPickedFiles = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    const { fileIds, makePublic } = req.body as {
      fileIds?: unknown;
      makePublic?: unknown;
    };
    ...
    if (!Array.isArray(fileIds) || fileIds.length === 0 || fileIds.length > MAX_PICKED_IDS) {
      return next(
        new ErrorResponse(
          `Send between 1 and ${MAX_PICKED_IDS} picked Drive file or folder IDs`,
          400,
        ),
      );
    }
    ...
    const importFromFolder = async (folderId: string, segments: string[]): Promise<void> => {
      if (visited >= MAX_VISITED_ITEMS) {
        throw new ErrorResponse(
          `Import aborted after visiting ${MAX_VISITED_ITEMS} Drive items. Pick smaller folders.`,
          400,
        );
      }
      let children: { files: { id?: string | null; name?: string | null }[]; folders: { id?: string | null; name?: string | null }[] };
      try {
    ...
    const importPickedId = async (id: string): Promise<void> => {
      let meta;
      try {
    ...
    try {
      ...
      return next(
        new ErrorResponse(
          "Failed to import from Drive. Check that an admin has connected Google.",
          500,
        ),
      );
    }
...
export { syncDrive, getSyncDebug, getPickerToken, importPickedFiles };